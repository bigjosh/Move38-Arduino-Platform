...
#define PULL_REQUEST_TIMEOUT_AFTER_PUSH_MS 250          // We will not start sending more pull request until this long after we sent a push
...
void blinkos_blinkboot_sendPushFlash( uint8_t face , uint8_t page ) {
    ...
    if (irSendBegin( face )) {
        ...
        displayedRawPixelSet.rawpixels[face] = rawpixel_t( 255 , 128 , 255 );
