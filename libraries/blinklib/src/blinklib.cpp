...
static void updateIRFaces(uint32_t now) {
    ...
    FOREACH_FACE(f) {
        ...
        if (irDataIsPacketReady(f)) {
            ...
            if ( packetBuffer[0] <= IR_DATA_VALUE_MAX ) {
                ...
                if ( irDataPacketLen( f ) == 2 ) {
                    ...
                    byte check = packetBuffer[1];

                    if ( checkByteTest( data , check ) ) {
                ...
                irPacketProcess( f , packetBuffer , irDataPacketLen(f) );
        ...
        if ( neighboorSendTime[f] <= now ) {        // Time to send on this face?
            ...
            packetBuffer[0] = data;
            packetBuffer[1] = checkByteMake( data );          // Invert byte as an error check for face values
