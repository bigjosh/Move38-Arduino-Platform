...
static void setupPixelPins(void) {
	...
	// Set the R,G cathode sinks to HIGH so no current flows (this will turn on pull-up until next step sets direction bit)..
    
	SBI( LED_R_PORT , LED_R_BIT );       // RED
	SBI( LED_G_PORT , LED_G_BIT );       // GREEN
    
    // We leave BLUE low so it will not enable pullup.     
    ...
    // We leave BLUE floating so no current will flow though boost cap
...
static void pixelTimersOn(void) {
    ...
    /*
     "Setting the OCR2A equal to MAX will result in a constantly high or low
     output (depending on the polarity of the output set by the COM2A[1:0] bits.)"
    */
    
    // ... so leaving OCR2B set to 255 will result in a constant high output
    
    TCCR2A =
        _BV( COM2B1) | _BV( COM2B0) |         // 1 1 = SET OC0B on Compare Match (charge boost), CLEAR OC0B at BOTTOM (slam down boost), (non-inverting mode)
...
>>>>>>> e22e817ce4744eead54a9e3b0ad33d50e5da7887
                ...
                SBI( LED_B_DDR , LED_B_BIT );               // Set blue PWM pin to output. The match is currently 255, so this will be LOW until
                                                            // The next PWM value gets loaded from the buffer at the start of the next phase
                                                                             
                OCR2B = currentPixel->rawValueB;            // This will get buffered and then run in next phase                                                             
                
            }  else {
                
                CBI( LED_B_DDR , LED_B_BIT );               // Float the blue PWM pin. The match is currently 255, so this will be LOW until
                                                            // and floating it stops any tiny current floating though any tiny residual charge on the
                                                            // cap and dimly lighting the blue LED when we activate this next pixel. 
...
>>>>>>> e22e817ce4744eead54a9e3b0ad33d50e5da7887
            ...
            // Bam, this will let loose the current to flow down though the blue LED into the cap.         
            activateAnode( currentPixelIndex );
                    
            OCR0A = currentPixel->rawValueR;    // Load OCR to turn on red at next overflow
                                                // Blue still might be on, but thats ok
                                                // because the RED has a lower forward voltage. 
...
static const uint8_t PROGMEM gamma8[32] = {
    255,254,253,251,250,248,245,242,238,234,230,224,218,211,204,195,186,176,165,153,140,126,111,95,78,59,40,19,13,9,3,0
};
...
void pixel_bufferedSetPixel( uint8_t pixel, pixelColor_t newColor) {
    ...
    rawpixel->rawValueB= 255- newColor.b*2; //pgm_read_byte(&gamma8[newColor.b]);
